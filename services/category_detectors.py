...
class NewsDetector(KeywordLoaderMixin, CategoryOverrideDetector):
    ...
    def detect(self, metadata: Dict, category_probs: list = None) -> Dict:
        """
        Detect if content is News & Politics
        
        Args:
            metadata: Video metadata dictionary
            category_probs: (unused) passed by caller for compatibility
        ...
        combined_text = f"{title} {channel} {description}"

        # PRIORITY 0: Law enforcement official channels → always News & Politics
        law_enforcement_channels = [
            'police department', 'police dept', 'sheriff', 'lapd', 'nypd',
            'department of justice', 'fbi', 'atf', 'dea', 'highway patrol',
            'state police', 'metropolitan police', 'city police', 'county sheriff',
            'police service', 'gendarmerie', 'carabinieri', 'interpol'
        ]
        if any(kw in channel for kw in law_enforcement_channels):
            print(f"  ✅ NewsDetector: Law enforcement channel detected → News & Politics")
            # Build updated category probs boosting News & Politics
            updated_cats = [{'category': 'News & Politics', 'probability': 0.95}]
            from .category_override_detector import DetectionResult
            return DetectionResult(
                is_detected=True,
                category='News & Politics',
                confidence=0.95,
                score=10,
                indicators=['law_enforcement_channel'],
                all_categories=updated_cats
            )

        # CRITICAL: Skip if combat sports keywords present
        combat_sports_keywords = [
            'wwe', 'wrestling', 'raw', 'smackdown', 'nxt',
            'ufc', 'mma', 'boxing', 'fight night',
            'wrestlemania', 'royal rumble', 'summerslam'
        ]
        has_combat_keywords = any(kw in combined_text for kw in combat_sports_keywords)
        if has_combat_keywords:
            print(f"  ⚠️  NewsDetector: Skipping - detected combat sports content")
            return {
                'is_detected': False,
                'score': 0,
                'max_score': 10,
                'indicators': [],
                'reason': 'Combat sports content excluded from news detection'
            }

        # Normal news detection
        result = super().detect(metadata)
        return result
...
class SportsDetector(CategoryOverrideDetector):
    ...
    def detect(self, metadata: Dict, category_probs: list = None) -> Dict:
        """
        Enhanced sports detection with weighted scoring
        ...
        # Combat sports keywords (highest priority)
        combat_keywords = [
            'ufc', 'wwe', 'wrestling', 'mma', 'boxing', 'kickboxing',
            'fight night', 'bellator', 'one championship', 'pfl',
            'wrestlemania', 'royal rumble', 'summerslam', 'raw', 'smackdown'
        ]
        
        # General sports keywords
        general_sports_keywords = [
            'football', 'basketball', 'soccer', 'baseball', 'hockey',
            'nfl', 'nba', 'mlb', 'nhl', 'championship', 'playoffs',
            'highlights', 'recap', 'game', 'match', 'vs', 'versus'
        ]
        
        # Score based on keyword matches and location
        score = 0
        indicators = []
        max_score = 10
        ...
        # Check combat sports (automatic high score)
        combat_matches = sum(1 for kw in combat_keywords if kw in combined_text)
        if combat_matches > 0:
            score += min(combat_matches * 3, 8)  # Max 8 points for combat
            indicators.append(f'combat_keywords ({combat_matches})')
            
            # Channel match = automatic detection (WWE channel = WWE content)
            if any(kw in channel for kw in combat_keywords):
                score = 10
                indicators.append('combat_sports_channel')
        
        # Check general sports
        general_matches = sum(1 for kw in general_sports_keywords if kw in combined_text)
        if general_matches > 0:
            score += min(general_matches, 3)  # Max 3 points for general
            indicators.append(f'sports_keywords ({general_matches})')
        
        # Duration analysis (sports events typically long)
        duration = metadata.get('duration', 0)
        if 900 <= duration <= 10800:  # 15 min to 3 hours
            score += 1
            indicators.append('typical_sports_duration')
        
        # Decision logic
        is_detected = score >= 4  # Threshold for override
        
        print(f"\n🏈 SPORTS DETECTOR ANALYSIS:")
        print(f"  Combat keyword matches: {combat_matches}")
        print(f"  General sports matches: {general_matches}")
        print(f"  Total score: {score}/10")
        print(f"  Indicators: {indicators}")
        print(f"  Override triggered: {is_detected}")
