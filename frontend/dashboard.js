...
async function analyzeVideo() {
    ...
    let framesPerMinute = parseInt(framesPerMinuteElement.value) || 10;
...
/**
 * Submit feedback (thumbs up/down) for violence or category prediction
 */
async function submitFeedback(type, isCorrect, userCorrection = null) {
    const analysisId = currentAnalysisId;
    if (!analysisId) return;
    
    // Get the predicted value for context
    let predictedValue = '';
    if (type === 'violence') {
        predictedValue = document.getElementById('feedbackViolenceValue')?.textContent || '';
    } else if (type === 'category') {
        predictedValue = document.getElementById('feedbackCategoryValue')?.textContent || '';
    }
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (thanks) {
                ...
                thanks.textContent = isCorrect 
                    ? '✅ Thanks! Marked as correct.' 
                    : '✅ Thanks! We\'ll improve this.';
            }
            // Hide correction box for category
            if (type === 'category') {
                const corrBox = document.getElementById('categoryCorrectionBox');
                if (corrBox) corrBox.style.display = 'none';
            }
...
/**
 * Show category correction dropdown when user clicks 'Wrong'
 */
function showCategoryCorrection() {
    const btns = document.getElementById('categoryFeedbackBtns');
    const corrBox = document.getElementById('categoryCorrectionBox');
    if (btns) btns.style.display = 'none';
    if (corrBox) corrBox.style.display = 'flex';
}

/**
 * Submit category correction with the user-selected correct category
 */
function submitCategoryCorrection() {
    const select = document.getElementById('categoryCorrectionSelect');
    const correction = select?.value;
    ...
    submitFeedback('category', false, correction);
}
