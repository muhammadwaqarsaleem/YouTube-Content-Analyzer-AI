...
class ViolenceDetectionService:
    ...
    def __init__(self, model_path: str = None):
        ...
        if gpus:
            print(f"🧠 [Violence Detection] Initialized on GPU: {gpus[0].name}")
        else:
            print(f"🧠 [Violence Detection] Initialized on CPU")
            
        # Auto-detect best available model
        if model_path is None:
            base_dir = str(Path(__file__).parent.parent)
            finetuned_path = os.path.join(base_dir, 'models', 'violence_detection', 'fine_tuned', 'final_model.h5')
            base_path = os.path.join(base_dir, 'models', 'violence_detection', 'violence_detection_model_resnet.h5')
